...
class Content_model extends CI_Model
{
	...
	function get_my_transactions_pinjam($uid, $limit=NULL, $start=NULL, $search=NULL)
	{
		$search_query = '';
	...
	function get_my_transactions_pendana($uid, $limit=NULL, $start=NULL, $search=NULL)
	{
		$search_query = '';
		if ($search != NULL && $search !='')
		{
	...
	function get_profil_pinjam_byid($id)
	{
		$this->db->select('*');
		$this->db->from($this->mod_agent_survey. ' mas');
		$this->db->where('mas.master_loan_id', $id);
	...
	{
		$this->db->select('kode_top_up, jml_top_up, tgl_top_up, status_top_up');
	...
	{
		$this->db->select('ltp_Master_loan_id, ltp_total_pinjaman, ltp_total_pinjaman_disetujui, ltp_jml_angsuran, ltp_lama_angsuran, ltp_tgl_jatuh_tempo, ltp_product_title, ltp_product_loan_term, ltp_product_pph, ltp_date_created');
	...
	function all_transactions_pinjam()
	{
	...
	// ----------- Survey ---------
	function insert_mod_agent_survey($data)
	{
		$this->db->insert($this->mod_agent_survey, $data);
		$id = $this->db->insert_id();
		return $id;
	}

	function update_mod_agent_survey($uid, $data)
	{
		$this->db->where('id_mod_agent_survey', $uid);
		$this->db->update($this->mod_agent_survey, $data);
		return $this->db->affected_rows();
	}

	function get_mod_agent_survey($id, $start, $limit)
	{
		$this->db->select('*');
		$this->db->from($this->mod_agent_survey. ' mas');
		$this->db->where('mas.id_agent', $id);
		$this->db->limit($limit, $start);
		$sql = $this->db->get();
		return $sql->result_array();
	}

	function set_mod_agent_survey_details($uid)
	{
		$this->db->select('*');
		$this->db->from($this->mod_agent_survey. ' mas');
		$this->db->where('mas.id_mod_agent_survey', $uid);
		$sql = $this->db->get();
		return $sql->result_array();
	}

	function get_total_mod_agent_survey($uid)
	{
		$this->db->select('count(*) as itotal');
		$this->db->from($this->mod_agent_survey);
		$this->db->where('id_agent', $uid);
		$queries = $this->db->get();
		$ret = $queries->row_array();
		return $ret;
	}
	...
	// ----------- Collection ---------
	function get_collection_approve($uid)
	{
		$sql = "SELECT 
			Master_loan_id as transaksi_id,
			Jml_permohonan_pinjaman as total_pinjaman, 
			Jml_permohonan_pinjaman_disetujui as total_pinjaman_disetujui, 
			Master_loan_status as transaksi_status, 
			tgl_pinjaman_disetujui as tgl_approve,
			product_title, Loan_term, type_business_name,
			nama_peminjam,
			Id_ktp as id_ktp,
			-- Alamat as alamat,
			User_id as id_peminjam
			...
			LEFT JOIN {$this->product} prod ON(prod.Product_id=p.Product_id)
			LEFT JOIN {$this->mod_type_business} tb ON(tb.id_mod_type_business=prod.type_of_business_id)
			LEFT JOIN {$this->user} u ON(u.Id_pengguna=p.pinjam_member_id)
			-- LEFT JOIN {$this->profile_geografi} g ON(g.User_id=p.pinjam_member_id)
			WHERE (tb.id_mod_type_business='3' OR tb.id_mod_type_business='4')
			AND p.pinjam_member_id ='{$uid}'
			AND p.Master_loan_status = 'approve'
			ORDER BY tgl_permohonan_pinjaman DESC
	...
	function insert_mod_agent_collection($data)
	{
		$this->db->insert($this->mod_agent_collection, $data);
		$id = $this->db->insert_id();
		return $id;
	}

	function insert_mod_agent_collection2($data)
	{
	...
	function get_mod_agent_collection($id, $start, $limit)
	{
		$this->db->select('*');
		$this->db->from($this->collection. ' c');
		$this->db->where('c.id_mod_agent', $id);
		$this->db->limit($limit, $start);
		$sql = $this->db->get();
		return $sql->result_array();
	}

	function get_mod_agent_collection_bymaster_loanid($master_loanid)
	{
		$this->db->select('*');
		$this->db->from($this->collection. ' c');
		$this->db->where('c.Master_loan_id', $master_loanid);
		$this->db->limit($limit, $start);
		$sql = $this->db->get();
		$ret = $sql->row_array();
		$sql->free_result();
	...
	function set_collection($uid)
	{
		$this->db->select('*');
		$this->db->from($this->collection. ' c');
		$this->db->where('c.Master_loan_id', $uid);
		$sql = $this->db->get();
		return $sql->row_array();
	}

	function set_collection_list($uid)
	{
		$this->db->select('c.Master_loan_id, c.tgl_collection, c.jml_tagihan');
		$this->db->from($this->collection. ' c');
		$this->db->where('c.Master_loan_id', $uid);
		$sql = $this->db->get();
		return $sql->result_array();
	...
	function get_total_mod_agent_collection($uid)
	{
		$this->db->select('count(*) as itotal');
		$this->db->from($this->collection);
		$this->db->where('id_mod_agent', $uid);
		$queries = $this->db->get();
		$ret = $queries->row_array();
		return $ret;
	}
