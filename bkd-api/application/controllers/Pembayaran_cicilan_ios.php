...
class Pembayaran_cicilan_ios extends REST_Controller {
		...
		if (Authorization::tokenIsExist($headers)) {
            ...
            if ($token != false) {
				...
				if (!empty($uid) && is_numeric($logintype)) {
					...
					if ($kode == 'PK')
					{
						redirect('pembayaran_cicilan_ios/submit_cicilan_kilat/'.$transaksi_id.'/'.$jml_bayar.'/'.$uid);
					}else{
						redirect('pembayaran_cicilan_ios/submit_cicilan_mikro/'.$transaksi_id.'/'.$jml_bayar.'/'.$uid);
					}
	...
	function submit_cicilan_kilat_get($transaksi_id, $jml_bayar, $uid)
	{
	...
	function submit_cicilan_mikro_get($transaksi_id, $jml_bayar, $uid)
	{
		if (!empty($uid) ) {
