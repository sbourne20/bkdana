...
class Agent_model extends CI_Model
{
	...
	function get_all_dt()
	{
		...
		$_sql_where 	= array();
		$sql_where 		= '';
		$cols 			= array( "id", "organizer_name", "organizer_address", "organizer_story", "organizer_mission", "organizer_note", "");
		$sort 			= "desc";
		...
		if (isset($_GET['sSearch']) && $_GET['sSearch'] != "" ) {
			...
			$_sql_where[] = "
				(
					UCASE(organizer_name) LIKE '%".$search."%'
				)
			";
		}
	...
	public function insert_($data)
	{
		...
		$this->db->insert($this->tabel_agent, $data);
	...
	{
		$this->db->where('id', $ID);
		$this->db->update($this->tabel_agent, $data);
		return $this->db->affected_rows();
	}
	
	public function delete_($id)
	{
		return $this->db->delete($this->tabel_agent, array('id'=>$id));
	}
