...
class Grade_model extends CI_Model
{
	...
	function get_all_dt()
	{
		...
		$_sql_where 	= array();
		$sql_where 		= '';
		$cols 			= array( "id_mod_grade_user", "grade_name", "completeness_profile", "");
		$sort 			= array("grade_name" => "asc");
		...
		if (isset($_GET['sSearch']) && $_GET['sSearch'] != "" ) {
			...
			$_sql_where[] = "
				(
					UCASE(grade_name) LIKE '%".$search."%' OR
					UCASE(completness_profile) LIKE '%".$search."%'
				)
			";
		}
	...
	public function insert_($data)
	{
		...
		$this->db->insert('mod_grade_user', $data);
		return $this->db->insert_id();
	}

	function get_grade_byid($id)
	{
		$this->db->select('*');
		$this->db->from('mod_grade_user');
		$this->db->where('id_mod_grade_user', $id);
		$this->db->limit(1);
		$sql = $this->db->get();
	...
	{
		$this->db->where('id_mod_grade_user', $ID);
		$this->db->update('mod_grade_user', $data);
		return $this->db->affected_rows();
	}
	
	public function delete_($id)
	{
		return $this->db->delete('mod_grade_user', array('id_mod_grade_user'=>$id));
	}
	...
	function checkif_exist($name, $id='')
	{
		...
		$this->db->select('*');
		$this->db->from('mod_grade_user');
		$this->db->where('UCASE(grade_name) = ', $name);
