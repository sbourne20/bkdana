...
class Peminjam extends CI_Controller {
	...
	public function editprofil()
	{
		...
		$output['kota'] = $this->Member_model->get_all_cities();
		$output['pendidikan'] = $this->Member_model->get_all_pendidikan();
		$output['agama'] = $this->Member_model->get_all_agama();
		$output['pekerjaan'] = $this->Member_model->get_all_pekerjaan();
		$output['nama_bank'] = $this->Member_model->get_all_bank();
		$output['status_tempat_tinggal'] = $this->Member_model->get_all_status_tempat();
		$output['gender'] = $this->Member_model->get_all_gender();
		//batas tambahan master option

		//$output['memberdataagama'] = $this->Member_model->get_usermember_agama_by();
		$output['membergroup'] = $this->Member_model->get_allgroup();
		$output['usergroup'] = $this->Member_model->get_usergroup();
		$usergroup = $this->Member_model->get_usermember_by($id);
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$output['top_css'] .= add_css('plugins/alertify/css/alertify.min.css');
			$output['top_css'] .= add_css('plugins/alertify/css/themes/default.min.css');
			$output['top_css'] .= add_css("plugins/fileinput/fileinput.min.css");
			//$output['top_js']    .= add_js("plugins/bootstrap-datepicker/dist/js/bootstrap-datepicker.min.js");
			$output['top_js']    .= add_js("plugins/bootstrap-timepicker/bootstrap-timepicker.js");
			$output['top_js']    .= add_js('plugins/ckeditor/ckeditor.js');
			...
			$output['top_js']    .= add_js("plugins/numeric/jquery.numeric.min.js");	
			$output['bottom_js'] .= add_js('js/select2-data.js');
			$output['bottom_js'] .= add_js('plugins/validationengine/languages/jquery.validationEngine-en.js');
			...
			$output['bottom_js'] .= add_js('plugins/alertify/alertify.min.js');
			$output['bottom_js'] .= add_js("plugins/fileinput/plugins/piexif.min.js");
			$output['bottom_js'] .= add_js("plugins/fileinput/fileinput.min.js");
			...
			$output['bottom_js'] .= add_js('plugins/fileinput-edit.js');
			//$output['bottom_js'] .= add_js('plugins/dsn.js');
			$output['bottom_js'] .= add_js('plugins/exif-js-master/exif.js');
			$output['bottom_js'] .= add_js('plugins/ImageTools.js');
			$output['bottom_js'] .= add_js('js/data/profile_edit.js');
			$output['bottom_js'] .= add_js('js/global.js');
			//batas tambahan
	...
	function kota_provinsi()
	{
		...
		$kota = $this->Member_model->getkota($usr);
