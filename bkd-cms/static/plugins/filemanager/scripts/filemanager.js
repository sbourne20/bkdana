...
var selectItem = function(data) {
	...
	var url = 'http://' + window.location.hostname + '/data/images/library/' + data['Path'].replace(fileRoot,"");
...
var getFileInfo = function(file) {
	...
	var template = '<div id="preview"><img height="200" /><h1></h1><dl></dl></div>';
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			...
			var gambar_preview_select = 'http://' + window.location.hostname + '/data/images/library/' + data['Path'].replace(fileRoot,"");

			$('#fileinfo').find('h1').text(data['Filename']).attr('title', file);
			$('#fileinfo').find('img').attr('src', gambar_preview_select);
			...
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(key in data){
					...
					if (data[key]['File Type'] == 'dir'){
						var gambar = 'images/fileicons/_Open.png';
					}else{
						var gambar = 'http://' + window.location.hostname + '/data/images/library/' + data[key]['Path'].replace(fileRoot,"");
					}

					result += '<li class="' + cap_classes + '"' + title + '"><div class="clip"><img src="' + gambar + '" width="' + scaledWidth + '" alt="' + data[key]['Path'] + '" data-path="' + data[key]['Path'] + '" /></div><p>' + data[key]['Filename'] + '</p>';
