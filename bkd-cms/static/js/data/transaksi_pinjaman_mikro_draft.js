  ...
  (function() {    
    ...
    var dataTable = $('#dynamic-table').dataTable( {
        ...
        "bServerSide" : true,
        "bStateSave"  : true,
        "sAjaxSource" : baseURL + "transaksi-pinjaman-mikro-draft/json",
        "columns": [
          { "data": "Master_loan_id" },
          { "data": "Master_loan_id" },
          { "data": "Nama_pengguna" },
          ...
          { "data" : function ( data, type, full, meta ) {
             var loan_status = data.Master_loan_status;
              if (loan_status=='draft' || loan_status=='approve' || loan_status=='lunas' || loan_status=='expired' || loan_status=='pending'  || loan_status=='complete') {
