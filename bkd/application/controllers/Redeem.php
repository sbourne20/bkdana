	...
	function submit_r()
	{
		if($_SERVER["REQUEST_METHOD"] == "POST")
		{
			...
			if (!empty($uid) && trim($uid) !='')
			{
				...
				{
					$this->session->set_userdata('message','Maaf, Transaksi Tarik Tunai Anda sebelumnya belum selesai.');
					$this->session->set_userdata('message_type','error');
					redirect('redeem');
					exit();
				}
				...
				{
					$this->session->set_userdata('message','Pengajuan Redeem Gagal. Jumlah minimum adalah Rp 100,000.');
					$this->session->set_userdata('message_type','error');
					redirect('redeem');
				}
